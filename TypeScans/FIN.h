...
// Perform a TCP FIN scan against the specified destination port
void fin_connect(int dest_port,string ipaddress){
...
// Send the crafted FIN packet to the target
...
for(int i=0;i<=1;i++){
    ...
    if(received < 0){
        ...
        if(errno == EAGAIN || errno == EWOULDBLOCK){
            ...
            // Store the port as open/filtered according to FIN scan behavior
