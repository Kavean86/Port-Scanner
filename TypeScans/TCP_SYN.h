...
// Perform a TCP SYN scan against the specified destination port
void syn_connect(int dest_port,string ipaddress){
...
// Keep the other TCP flags disabled
tcp->rst=0;
tcp->fin=0;
tcp->ack=0;
tcp->urg=0;
...
// Send the crafted TCP SYN packet to the target
...
for(int i=0;i<=1;i++){
    ...
    if(received < 0){
        ...
        // Stop waiting if the receive operation timed out
...
// Check whether the response contains SYN and ACK flags
// and verify that the source and destination ports match
if(tcprecv->syn==1 && tcprecv->ack==1 && ntohs(tcprecv->source)==dest_port && ntohs(tcprecv->dest)==source_port){
